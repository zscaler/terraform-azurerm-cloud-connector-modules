...
################################################################################
# 2. Create Cloud Connector Virtual Machine Scale Sets (VMSS) registered into
#    the GWLB backend pool. Optionally, deployed VMSS instances can span
#    designated availability zones if enabled via "zones_enabled" and "zones"
#    variables, where the number of VMSS created will equal the number of
#    zones specified. E.g. 2 zones ["1","2"] and vmss_default_ccs of 2 will
#    create 2x Scale Sets EACH with 2x CCs where VMSS-1 CCs are assigned AZ1
#    and VMSS-2 CCs in AZ2.
################################################################################
...
################################################################################
# 3. Create Function App and dependencies for VMSS lifecycle management
################################################################################
...
#    service interface(s). VMSS deployments share a single NSG across all CC
#    instances in each Scale Set.
################################################################################
module "cc_nsg" {
...
#    all Cloud Connectors and the Function App
################################################################################
module "cc_identity" {
...
################################################################################
# 6. Create Azure Gateway Load Balancer in CC subnet with VXLAN tunnel
...
################################################################################
# 7. Optionally create a consumer Public Load Balancer chained to the GWLB.

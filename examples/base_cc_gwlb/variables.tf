...
################################################################################
# Gateway Load Balancer (GWLB) Configuration
################################################################################
...
variable "vxlan_external_port" {
  ...
  validation {
    condition = (
      var.vxlan_external_port >= 1024 && var.vxlan_external_port <= 65535
    )
...
variable "vxlan_internal_port" {
  ...
  validation {
    condition = (
      var.vxlan_internal_port >= 1024 && var.vxlan_internal_port <= 65535
    )
...
variable "vxlan_external_vni" {
  ...
  validation {
    condition = (
      var.vxlan_external_vni >= 1 && var.vxlan_external_vni <= 16777215
    )

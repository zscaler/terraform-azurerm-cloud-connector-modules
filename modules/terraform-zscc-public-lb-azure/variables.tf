...
variable "gateway_load_balancer_frontend_ip_configuration_id" {
  type        = string
  description = "The ID of the Gateway Load Balancer frontend IP configuration to chain this Public Load Balancer to. When set, all traffic through this PLB frontend will be transparently redirected through the GWLB (and its CC VMs) before reaching the backend. Leave null to disable GWLB chaining."
  default     = null
}
